class CloudinaryImageMixin {
  constructor(selectors, uploadSuccessfulCallback) {
    ...
    this.options = {
      thumbnailWidth: 100,
      addAssetButton: selectors.addAssetButton || '.cloudinary-asset-btn',
      hasMaxWidthMaxHeight: selectors.hasMaxWidthMaxHeight
    };
  ...
  cleanUp () {
    $(window).one('close-modal', () => {
      $(window).off('dragDrop.addImage');
      $(window).off('dragDrop.refresh');
      $(window).off('assetSelected.addImage');
      $('.fa-crop').off('click.imageEditor');
      $('.cloudinary-remove-btn').off('click.imageEditor');
      $(this.selectors.primary).off('keyup.refresh');
      $(this.selectors.primary).off('keydown.refresh');
    })
  }

  setupAddingImages() {
    $(this.options.addAssetButton).bind('click', (e)=>{
      this.definePreviousValues();
      let tabContent = $(e.target).closest('.tab-content'),
          tabHref = `#${tabContent.attr('id')}`;
      this.nthTabItem = tabContent.parent().children('div').index($(tabHref));
      let {previousValues} = $(e.currentTarget).data();
      $(window).trigger('add-assets-button-clicked', [e, previousValues]);
    });
    $(window).on('dragDrop.addImage', ()=>{this.renderThumbs();});

    $(window).on('assetSelected.addImage', (_, assets, inputSelector, previousWidgetSettings) => {
      this.setInputValues(previousWidgetSettings);
      if (assets && assets.length) {
        $('ul.tabs li').eq(this.nthTabItem).find('a').click(); // change to the tab the user previously had open
  ...
  renderThumbs(inputID) {
    var renderThumb = (i, e) => {
      setTimeout(() => {
        ...
        var placeholderURL = "https://via.placeholder.com/100x100",
...
class G5ImageValidation {
  constructor() {
    this.submitBtn = $('.modal-action.g5-modal-save-btn');
  }

  setErrorMessage(errorMessage, input) {
    this.removeErrorMessage(input);
    input.addClass('invalid').prop('invalid', true);
    $(`<label class="error">${errorMessage}</label>`).insertAfter(input);
  }

  removeErrorMessage(input) {
    const errorSelector = `#${input.attr('id')} + label.error`;
    $(errorSelector).remove();
    input.removeClass('invalid').removeProp('invalid');
  }

  validationError(errorMessage, input) {
    input[0].setCustomValidity(errorMessage);
    this.setErrorMessage(errorMessage, input);
  ...
  validAltTag(url, alt) {
    ...
    const altValue = alt.val();

    return !urlValue || !!altValue;
  }

  toggleSubmit(enable = true) {
    this.submitBtn.attr('disabled', () => enable ? null : true);
  }

  validateUrl(primary) {
    $(primary).parents('form').attr('front-end-validation', 'enabled');

    $(primary).each((index, item) => {
      const input = $(item);

      input.on('blur', () => {
        const inputSelector = `#${input.attr('id')}`;
        const errorMessageSelector = `${inputSelector} + label.error`;
        const validatingMessageSelector = `${inputSelector} + label.info`;
        const value = input.val().trim();
        const liquidBypass = value.match(/^{{content_.+}}$/);
        input.val(value);

        $(errorMessageSelector).remove();

        // Don't try to validate when blank or using liquid that looks like {{content_location_whatever}}
        if(!value || liquidBypass) {
          this.resetValidation(input);
          return;
        }

        this.toggleSubmit(false);
        $(`<label class="info">Validating image url</label>`).insertAfter(input);

        var img = new Image();
        img.addEventListener('load', () => {
          this.resetValidation(input);
          $(validatingMessageSelector).remove();
        }, false);

        img.src = value;

        img.onerror = () => {
          const errorMessage = 'Invalid image URL';
          $(validatingMessageSelector).remove();
          input[0].setCustomValidity(errorMessage);
          this.setErrorMessage(errorMessage, input);
          this.toggleSubmit();
  ...
  validateAltTag(urlAltPairs) {
    const blankAltErrorMessage = 'Alt Tag must not be blank';
    const changedUrlMessage = "URL has changed. Please make sure to update Alt Tag so it's relevant to the new image";

    urlAltPairs.forEach((inputs) => {
      const url = $(inputs[0]);
      if (!url) { return false; }

      const originalUrlValue = url.val();
      const alt = $(inputs[1]);

      if (!this.validAltTag(url, alt)) {
        this.setErrorMessage(blankAltErrorMessage, alt);
        this.toggleSubmit(false);
      }

      alt.on('keyup', this.debounce(() => {
        this.validAltTag(url, alt) ? this.resetValidation(alt) : this.validationError(blankAltErrorMessage, alt);
      }, 1000))

      url.on('change', () => {
        if (this.validAltTag(url, alt)) {
          this.removeErrorMessage(alt);

          if (originalUrlValue !== url.val() ) {
            this.setErrorMessage(changedUrlMessage, alt);
          }
        }
      })
    })
...
class G5ImageEditor extends CloudinaryImageMixin {
  constructor(options, onSuccessCallback) {
    ...
    const validation = new G5ImageValidation();

    if (this.validateImageUrl) {
      validation.validateUrl(this.primary);
    }

    if (this.validateAltTag) {
      validation.validateAltTag(this.urlAltPairs);
    }
  }

  setUpImageEditor() {

    $('.fa-crop').on('click.imageEditor', (e)=>{
    ...
    $('.cloudinary-remove-btn').on('click.imageEditor', (e) => {
      var closestFieldSet = $(e.currentTarget).closest('fieldset');
  ...
  setupRefreshOnTyping() {
    ...
    $input.on('keyup.refresh', inputChange);
    $input.on('keydown.refresh', () => {
      clearTimeout(typingTimer);
    });
    $input.change(inputChange);

    $(window).on('dragDrop.refresh', inputChange);
  ...
  onChange({h, w, x, x2, y, y2}) {
    ...
    [x, x2, y, y2] = [x, x2, y, y2].map(Math.round);
    [h, w] = [h, w].map((v) => (v === 0) ? 1 : Math.round(v));
  ...
  setupButtonListeners(closestFieldSet) {
    var closeImage = () => {
      ...
      this.renderThumbs();
      $(window).trigger('closed-image-cropper');
    }
